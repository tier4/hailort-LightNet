...
DEFINE_string(hef, "hefs/lightNet-BDD100K-p50-semseg-depth-lk-noDilation-960x960_opt_a16w16_reg.hef",
              "HEF Path, "
              "HEF Path");
...
DEFINE_string(v, "test_video.mp4",
              "Video Path, "
              "Video Path");

DEFINE_int64(cam_id, -1, "Camera ID");

DEFINE_uint64(c, 10, "[OPTIONAL] num of classes for the inference engine.");
...
DEFINE_bool(preproc, true,
            "[OPTIONAL] Flag to whether needs preprocessing for Hailo inputs");
std::string
get_hef_path(void)
{
  return FLAGS_hef;
...
std::string getDepthColorFormat() { return FLAGS_depth;}

bool isRequiredPreproc(void)
{
  return FLAGS_preproc;
}
